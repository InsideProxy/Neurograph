...
## H6. `.npmrc` del proyecto contra ataques a la cadena de suministro de npm -- 26/09/2026

**Qué.** `frontend/.npmrc`, comentado en castellano, con `ignore-scripts=true` (los scripts de instalación de las dependencias no se ejecutan) y `min-release-age=3` (con npm >= 11.10, rechaza instalar una versión publicada hace menos de tres días). El paso de preparación (H5) avisa, en una línea aparte después de la de instalación y solo cuando instala algo, si el npm que lo ejecuta es anterior a 11.10.0 y por tanto ignora `min-release-age` en silencio; lee la versión de `npm_config_user_agent`, sin ejecutar `npm --version`. El parseo del user-agent y la comparación de versión son funciones puras con sus tests, en `frontend/scripts/npm-version.mjs`.

**Por qué.** El registro de npm sufrió varios ataques a la cadena de suministro en 2025-2026, todos a través de scripts de instalación (`preinstall`/`postinstall`): Shai-Hulud (09/2025), chalk/debug (09/2025), axios (31/03/2026), TanStack (05/2026) y el gusano de keyv «ChainDrop» (04-05/08/2026); las versiones maliciosas se retiraron en horas (axios ~4 h, chalk ~2,5 h, Shai-Hulud ~12 h), de ahí el valor de `min-release-age`. El usuario decidió mantener npm -- no migrar a pnpm por ahora; pnpm 11, con valores por defecto más seguros, queda como propuesta para el desarrollador principal -- y proteger el proyecto con un `.npmrc` propio. Su `~/.npmrc` personal ya tiene `ignore-scripts=true` y todo funciona con él.

**Verificación.** Tests nuevos de `frontend/scripts/npm-version.mjs` (parseo del user-agent y comparación de versión: npm 10.9.0, 11.10.0, 11.20.0, 12.x, pnpm, yarn y sin la variable) y del aviso compuesto en `frontend/scripts/npm-deps.mjs`; los 740 anteriores siguen en verde (752 en total). Con el npm 10.9.0 real de este equipo: el `.npmrc` del proyecto no produce ningún aviso propio de `min-release-age` (`npm install --dry-run` no avisa: npm 10 la ignora en silencio) y, con `ignore-scripts=true`, `npm run <script>` sigue ejecutando su propio comando -- solo se saltan los hooks pre/post y los scripts de instalación. Auditoría de `driver.js` 1.8.0, la única dependencia de producción añadida recientemente: publicado el 17/07/2026 por su autor (`kamranahmedse`), sin versión posterior ni siquiera tras el gusano de 08/2026; su sha512 coincide con el del registro y con el del lockfile; `npm audit signatures` verifica 154/154 paquetes (39 con «attestations»); `npm audit` da 0 vulnerabilidades; su `package.json` no declara `dependencies` ni scripts de instalación.
