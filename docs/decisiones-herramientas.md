...
## H5. El frontend instala solo las dependencias de npm que falten -- 26/09/2026

**Qué.** `frontend/scripts/ensure-deps.mjs` compara `frontend/package-lock.json` con el lockfile oculto que npm escribe al instalar (`node_modules/.package-lock.json`). Si falta un paquete, o cambian su versión, su `resolved` o su `integrity`, ejecuta `npm install --no-audit --no-fund`; si no, sale sin decir nada. Un opcional que falte no cuenta: npm deja fuera los de otras plataformas y los que fallan, y el lockfile no guarda todo lo que usa para decidirlo (en una instalación de Linux con glibc faltaba `@tauri-apps/cli-linux-x64-musl` y estaban las otras variantes musl). Lo ejecutan `predev` y `prebuild` antes de `npm run dev` y `npm run build`, también desde Tauri, salvo con `ignore-scripts=true` en npm, que se salta esos scripts.
**Por qué.** El usuario decidió que, tras un pull, arrancar la app instale sola las dependencias de npm que falten, sin ejecutar `npm install` en cada arranque si ya están.
**Verificación.** 8 pruebas nuevas, y cada cambio de la regla hace fallar alguna. En una copia sin red, con paquetes locales: instala sin `node_modules`, actualiza tras un pull que cambia una versión y no repite; un opcional de Windows no hace reinstalar, y el error de npm se propaga. Al día, 19 ms (92 ms con `npm run`). Con `ignore-scripts=true`, npm 10.9 no ejecuta `predev` ni `prebuild`. Tras `npm install --package-lock-only`, que reescribe también el lockfile oculto, la comprobación no ve lo que falta. Sin probar en Windows.
