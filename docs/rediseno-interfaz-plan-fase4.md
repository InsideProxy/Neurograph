...
2. **Espacio de las etiquetas.** El radio del círculo sigue siendo `lado / 2 − 40`, el del desarrollador principal. En HCP-MMP1.0 (letra de 5,5 px) la etiqueta más larga acaba a unos 31 px del anillo: cabe. En los atlas de abreviaturas largas no: calculado con sus abreviaturas, unos 51 px en Brainnetome («L_MVOcC _5_1», así, con un espacio, en los datos), 75 en Gordon 333 («r_frontoparietal_20») y 130 en el Subcórtex (19 regiones, letra de 9 px). Hoy ya se cortan por los lados; al girar, se cortan también arriba y abajo. No se encoge el círculo, que cambiaría la geometría y apretaría los nodos: lo decide el usuario (ver abajo).
...
6. **Leyenda (5.4).** `ConnectogramLegend`, una lista con nombre («Leyenda del connectograma») y cuatro entradas con su muestra en un `<svg>` decorativo. La muestra discontinua lleva el `dash` del tema, el de las líneas; las muestras son del color del texto (`--text`), y la leyenda, de `--text-muted`, a 0,7rem. Va dentro del hueco del dibujo (`.connectogram-area`, que pasa a `position: relative`), abajo a la izquierda, fuera del `<svg>`: no se exporta. Solo en la vista grande. Sin eventos (`pointer-events: none`), así que los nodos de debajo siguen recibiendo el ratón, también con la lupa. Fondo del panel al 84 % (`color-mix`), como el `scrim` de la maqueta, con respaldo opaco si `color-mix()` no existe.
   - **Tapa parte del dibujo.** En la maqueta, el círculo es más pequeño respecto a su lado (radio 0,39 del lado) y la leyenda cabía en la esquina. En la app es de 0,43 a 0,47 (`lado / 2 − 40`), y a 1400 × 900 la leyenda tapa, calculado, más de una docena de nodos del cuadrante de abajo a la izquierda, con sus etiquetas. La Task 9 lo mide a cuatro tamaños. Se sigue el spec y la maqueta, y lo decide el usuario (ver abajo).
...
Por diseño, donde cambia el dibujo:

- **Connectograma:** las etiquetas, giradas; con HCP-MMP1.0, los dos arcos con sus rótulos (con Brainnetome, Gordon 333 y el Subcórtex, no); y, con una selección, el halo de cada región seleccionada, con el color de selección de la exportación (`#16181c` con los temas 2 a 4, `#ac61d1` con Original) al 35 %. Los nodos, las líneas, sus colores y sus posiciones son los de antes. La leyenda no entra, porque va fuera del `<svg>`, y las marcas siguen sin entrar.
- **Hemisferios:** nada. `Hemisferios.tsx` no cambia.
- **Leyenda de la selección múltiple:** nada.
- **Cerebro 3D:** el sombreado de la corteza (surcos), en todos los temas, también en Original; y las etiquetas, con la tipografía nueva y la pastilla con los colores de exportación.
