...
- **Tipo de conectividad:** junto a cada tipo, cuántas conexiones de ese tipo pasan los demás filtros (redes y peso mínimo), sin contar su propia casilla. Así se ve cuántas añadiría al mostrarlo. `App` calcula esos recuentos y se los pasa a `FilterPanel` en una prop nueva, `connectionCountsByType`.
...
- **Automático:** 8 s por paso, y en los largos, 250 ms por palabra hasta 12 s: el tour entero dura 1 min 52 s. Se para en el último paso.
...
- **Lupa:** es el único paso en que el connectograma señalado admite el ratón, para probarla. Como el ejemplo tiene redes ocultas, el texto no da por hechos los 5 px: dice que serían con las 360 regiones a la vista.
